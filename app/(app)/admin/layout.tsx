...
import { LayoutDashboard, Building2, Megaphone, Heart, Calendar, BookOpen, HandCoins, PersonStanding, Users, Radio, Mic } from 'lucide-react'
...
const navItems = [
  ...
  { href: '/admin/bilal',         icon: Mic,              label: 'Bilal',      superadminOnly: false },
...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // Allow access if: global ajk/superadmin role OR mosque_ajk member for any mosque
  const hasMosqueRole = await (async () => {
    if (profile && ['ajk', 'superadmin'].includes(profile.role)) return true
